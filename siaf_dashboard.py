...
# Mapeo sec_func → área
SEC_FUNC_MAP = {
    1: "PI_2", 2: "DCEME", 3: "DE", 4: "PI_1", 5: "OPP", 6: "JEF", 7: "GG", 8: "OAUGD",
    9: "OTI", 10: "OA", 11: "OC", 12: "OAJ", 13: "RRHH", 14: "OCI", 15: "DCEME15",
    16: "DETN16", 21: "DETN21", 22: "DETN22",
}
...
def load_data(excel_bytes, usecols, sheet_name, header_row_excel, autodetect=True):
    ...
    if "sec_func" in df.columns:
        df["sec_func"] = df["sec_func"].apply(lambda x: SEC_FUNC_MAP.get(x, SEC_FUNC_MAP.get(str(x), x)))
    return df, s
...
def build_classifier_columns(df):
    ...
    df["clasificador_desc"] = (
        df["generica_desc"].fillna("") + " > " + df["subgenerica_desc"].fillna("") + " > " +
        df["subgenerica_det_desc"].fillna("") + " > " + df["especifica_desc"].fillna("") + " > " +
...
group_options = [c for c in df_proc.columns if c in [
    "clasificador_cod","unidad_ejecutora","fuente_financ","generica","subgenerica",
    "subgenerica_det","especifica","especifica_det","funcion","division_fn","grupo_fn",
    "programa_pptal","producto_proyecto","activ_obra_accinv","meta","sec_func","area"
]]
...
if "avance_%" in pivot.columns:
    pivot_disp = pivot.style.applymap(lambda v: "background-color:#ffcccc" if v < float(riesgo_umbral) else "", subset=["avance_%"])
else:
    pivot_disp = pivot
...
if "avance_%" in consol_display.columns:
    consol_display = consol_display.style.applymap(lambda v: "background-color:#ffcccc" if v < float(riesgo_umbral) else "", subset=["avance_%"])
...
if dev_cols and "mto_pim" in df_proc.columns:
    ...
    avance_series = dev_series[["mes","contrib_pct"]]
    ...
    chart = (
        ...
        .encode(
            ...
            tooltip=[alt.Tooltip("mes:O"), alt.Tooltip("monto:Q", title="Devengado", format=",.2f"),
                     alt.Tooltip("contrib_pct:Q", title="% contrib.", format=".2f")]
        )
